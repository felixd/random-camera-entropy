...
import app.control.control_server as control_server
from app.reporting.spatial_docs import discover_documents, render_markdown
...
def main() -> int:
    ...
    assert control_server.ALLOWED_PROFILES["spatial-campaign"] == "scripts/smoke/smoke_spatial_profiles.sh"
    ...
    assert 'href="/live?job=${encodeURIComponent(j.id)}"' not in template
    assert "$('clearLog').onclick=()=>{$('log').textContent=''}" in template
