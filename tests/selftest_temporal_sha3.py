...
from app.core import camera_entropy_server as server
from app.control.profile_catalog import ALLOWED_PROFILES


def main() -> int:
    root = Path(__file__).resolve().parents[1]
    profile = (root / "scripts/smoke/smoke_temporal_sha3.sh").read_text(encoding="utf-8")
    runner = (root / "scripts" / "run" / "run_one.sh").read_text(encoding="utf-8")
    control = (root / "app" / "control" / "control_server.py").read_text(encoding="utf-8")
    control_html = (root / "app" / "web" / "templates" / "control.html").read_text(encoding="utf-8")
    worker_source = (root / "app" / "core" / "camera_entropy_server.py").read_text(encoding="utf-8")
    ...
    assert ALLOWED_PROFILES["temporal-sha3"] == "scripts/smoke/smoke_temporal_sha3.sh"
