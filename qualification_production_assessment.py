...
APP_VERSION = "2026.08.04.camera-entropy-production-assessment.7.13.0"
MAX_LSB_BITS = 4
...
def main() -> int:
    ...
    for index, case in enumerate(cases, 1):
        ...
        with log_path.open("w", encoding="utf-8", errors="replace") as log_stream:
            child = subprocess.Popen([str(ROOT / "run_one.sh")], cwd=ROOT, env=environment, stdout=subprocess.PIPE, stderr=subprocess.STDOUT, text=True, bufsize=1)
        ...
        subprocess.run([sys.executable, str(ROOT / "summarize_production_assessment.py"), str(campaign_dir)], cwd=ROOT, check=False, stdout=subprocess.DEVNULL)
        if returncode != 0 and os.environ.get("ASSESSMENT_CONTINUE_ON_ERROR", "1") != "1":
            break

    subprocess.run([sys.executable, str(ROOT / "summarize_production_assessment.py"), str(campaign_dir)], cwd=ROOT, check=True)
