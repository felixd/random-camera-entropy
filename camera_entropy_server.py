        ...
        try:
            ...
            while not self.stop_event.is_set():
                ...
                if not self.warmup_complete:
                    ...
                    self.last_processed_monotonic = None
                    self.logger.info(
                        "Thermal warm-up complete after %.3f seconds; calibration begins",
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Distributed temporal entropy processor for V4L2, mTLS Y8 and RTSP sources"
    )
    parser.add_argument(
        "--source-type",
        choices=("v4l2", "tls-y", "rtsp"),
        default="v4l2",
        help="Frame input: local V4L2, remote raw-Y mTLS agent, or decoded RTSP luma",
    )
    ...
    if args.source_type == "rtsp" and args.manual_exposure:
        # RTSP control is camera/vendor-specific and is intentionally not guessed.
